...
- **Tests**: new unit tests (byte-exact prompt contracts, provider defaults/migration, fallback chain, session pinning) and model-gated integration tests (real Russian→Chinese inference, both models load/switch/release, missing-model semantics — auto-skip when models are absent); the full existing unit suite passes.
