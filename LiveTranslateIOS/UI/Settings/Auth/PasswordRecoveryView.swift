...
/// (the server never reveals whether the account exists); the user pastes
...
struct PasswordRecoveryView: View {
    ...
    private var requestSection: some View {
        Section {
            ...
            Text(String(localized: "如果该邮箱已注册，你将收到一封包含重置凭证的邮件。为保护账号安全，无论邮箱是否存在，界面显示都相同。"))
        }
    }
    ...
    // MARK: - Step 2: paste token + set new password

    private var resetSection: some View {
        Section {
            Text(String(localized: "重置邮件已发送到 \(email)。复制邮件中的重置凭证并粘贴到下方。"))
                .font(.subheadline)
            ...
            Text(String(localized: "重置后所有已登录设备都会被退出，需要用新密码重新登录。凭证 30 分钟内有效且只能使用一次。"))
        }
    }
